...
def extract_data(use_ICU:str, label:str, time:int, icd_code:str, root_dir, disease_label, cohort_output=None, summary_output=None):
    ...
    directory = os.path.join(root_dir, "data", "cohort")
    # Check if the directory exists, and create it if it doesn't
    if not os.path.exists(directory):
        os.makedirs(directory)
    # Save the cohort DataFrame as a compressed CSV
    cohort[cols].to_csv(os.path.join(directory, f"{cohort_output}.csv.gz"), index=False, compression='gzip')
